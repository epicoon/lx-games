...
class GeometryRing extends THREE.Geometry {
    constructor(r, height, depth, angle, sectors) {
        super();
        const geom = _calc(r, height, depth, angle, sectors);
...
function _calc(r, height, depth, angle, sectors) {
    sectors = sectors || lxGames.threed.SMOOTH;
    let step = angle / sectors,
        innerR = r - depth,
        vertices = [];
    ...
    for (let i=0; i<sectors*4; i+=4) {
        ...
        // For textures
        let x1 = (parseFloat(i)/4 + 1.0) / parseFloat(sectors),
            x2 = (parseFloat(i)/4) / parseFloat(sectors);
        uvs.push([]);
